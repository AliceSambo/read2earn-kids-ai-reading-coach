...
const screens = ['welcome','setup','map','mission','reader','comprehension','reward','report'];
...
try {
  ...
  const retryFeedback = await fetch(`http://127.0.0.1:${port}/api/comprehension`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ answer:'Nia walked in the forest.' }) });
  ...
  if (!result.understood || result.evidence.length < 2) throw new Error('Comprehension fallback did not recognise story evidence');
