...
try {
  ...
  if (!await page.locator('.mission-stage').isVisible() || await page.locator('.mission-objectives span').count() !== 3) throw new Error('Story Forest mission portal is incomplete.');
  await page.screenshot({ path: join(outputDir, 'story-forest-mission-desktop.png'), fullPage: true });
  await page.getByRole('button', { name: /read with my companion/i }).click();
  if (await page.locator('#readerScene').getAttribute('data-moment') !== '1') throw new Error('Reader did not begin at story moment one.');
  await page.screenshot({ path: join(outputDir, 'story-forest-reader-moment-1.png'), fullPage: true });
  await page.evaluate(() => Object.defineProperty(window, 'speechSynthesis', { configurable: true, value: undefined }));
  await page.getByRole('button', { name: /story glossary/i }).click();
  const glossary = page.getByRole('dialog', { name: /story glossary/i });
  await glossary.waitFor();
  if (await glossary.locator('.glossary-entry').count() !== 3) throw new Error('Story glossary did not include all outlined words.');
  await glossary.getByText(/shone with a soft, moving light/i).waitFor();
  await glossary.getByRole('button', { name: /hear shimmered/i }).click();
  await page.getByText(/spoken narration is not supported here/i).waitFor();
  await page.screenshot({ path: join(outputDir, 'story-glossary-desktop.png'), fullPage: true });
  await glossary.getByRole('button', { name: /close story glossary/i }).click();
  await page.getByRole('button', { name: /read this part/i }).click();
  await page.getByRole('button', { name: /next part/i }).click();
  await page.getByRole('button', { name: /next part/i }).click();
  if (await page.locator('#readerScene').getAttribute('data-moment') !== '3' || await page.locator('#reader-title').textContent() !== 'Waiting with patience') throw new Error('Story scene did not progress with the reading.');
  await page.getByRole('button', { name: 'patiently' }).click();
  await page.locator('#helpMeaning').getByText(/in a calm way while giving something enough time/i).waitFor();
  await page.getByRole('button', { name: /next part/i }).click();
  if (await page.locator('#readerScene').getAttribute('data-moment') !== '4') throw new Error('Restored forest scene was not reached.');
  await page.screenshot({ path: join(outputDir, 'story-forest-reader-restored.png'), fullPage: true });
  await page.getByRole('button', { name: /tell what happened/i }).click();
  if (await page.locator('.evidence-steps li').count() !== 3 || await page.locator('#evidenceOrbit').getAttribute('data-complete') !== '0') throw new Error('Comprehension evidence trail did not begin with three unearned clues.');
  if (await page.locator('#questionProgressLabel').textContent() !== 'QUESTION 1 OF 3' || await page.locator('.question-path .is-current').textContent() !== '1') throw new Error('Question-by-question flow did not begin at question one.');
  await page.getByPlaceholder(/lumi the firefly/i).fill('The light was safe');
  await page.getByRole('button', { name: /check my first answer/i }).click();
  await page.getByRole('button', { name: /try this question again/i }).waitFor();
  if (await page.locator('#attemptCount').textContent() !== '1') throw new Error('Per-question attempt marker did not update.');
  ...
  if (await page.locator('#evidenceOrbit').getAttribute('data-complete') !== '0') throw new Error('Weak retelling falsely illuminated evidence.');
  await page.screenshot({ path: join(outputDir, 'comprehension-needs-clues.png'), fullPage: true });
  await page.getByRole('button', { name: /try this question again/i }).click();
  await page.getByPlaceholder(/lumi the firefly/i).fill('Lumi the firefly');
  await page.getByRole('button', { name: /check my first answer/i }).click();
  await page.getByRole('button', { name: /next question/i }).click();
  if (await page.locator('#questionProgressLabel').textContent() !== 'QUESTION 2 OF 3' || await page.locator('#evidenceOrbit').getAttribute('data-complete') !== '1') throw new Error('First answer did not advance exactly one question.');
  await page.getByPlaceholder(/nia helped by/i).fill('Nia waited patiently and helped gently');
  await page.getByRole('button', { name: /check my second answer/i }).click();
  await page.getByRole('button', { name: /next question/i }).click();
  if (await page.locator('#questionProgressLabel').textContent() !== 'QUESTION 3 OF 3' || await page.locator('#evidenceOrbit').getAttribute('data-complete') !== '2') throw new Error('Second answer did not advance exactly one question.');
  await page.getByPlaceholder(/after nia helped/i).fill('The lantern was restored and the forest path became safe');
  await page.getByRole('button', { name: /check my final answer/i }).click();
  await page.getByRole('button', { name: /see my knowledge gem/i }).waitFor();
  if (await page.locator('#evidenceOrbit').getAttribute('data-complete') !== '3') throw new Error('Three individual answers did not assemble all three evidence clues.');
  if (await page.locator('.evidence-steps li.is-found').count() !== 3) throw new Error('Evidence trail did not visibly mark all three clues.');
  await page.screenshot({ path: join(outputDir, 'comprehension-all-clues.png'), fullPage: true });
  await page.getByRole('button', { name: /see my knowledge gem/i }).click();
  await page.getByRole('heading', { name: /three clues became a knowledge gem/i }).waitFor();
  await page.screenshot({ path: join(outputDir, 'knowledge-gem-reveal.png'), fullPage: true });
