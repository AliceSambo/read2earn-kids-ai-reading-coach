...
try {
  ...
  await page.getByRole('button', { name: /check the clues/i }).click();
  await page.getByRole('button', { name: /add the missing clue/i }).waitFor();
  if (await page.locator('#attemptCount').textContent() !== '1') throw new Error('Comprehension attempt marker did not update.');
  ...
  await page.getByRole('button', { name: /add the missing clue/i }).click();
  ...
  if (await page.locator('#attemptCount').textContent() !== '2' || await page.locator('#evidenceOrbit').getAttribute('data-complete') !== '3') throw new Error('Complete retelling did not assemble all three evidence clues.');
