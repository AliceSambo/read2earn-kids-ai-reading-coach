...
The server binds to `HOST` (default `0.0.0.0`) and uses the host-provided `PORT`, which makes the existing Node process suitable for a managed web service. `GET /health` returns `{ "status": "ok" }` without reading child data or contacting OpenAI. The deterministic three-category gate remains authoritative even when AI enhancement is enabled.

GitHub Actions runs the dependency-free test suite and whitespace validation on pushes and pull requests to `main`. CI intentionally has no OpenAI credential: this verifies that the fallback-only demo remains complete and reliable.
